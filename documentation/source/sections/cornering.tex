...
\codeword{caseSetup} can set up a steady curved-path (cornering) case using a Single Reference Frame (SRF). Instead of meshing a curved tunnel, the existing rectangular domain is solved in a frame that rotates at a constant angular velocity about a vertical axis through the corner centre. In that rotating frame the car follows a circular path of radius \codeword{CORNER_RADIUS} at the freestream speed \codeword{INLET_MAG}, and the steady flow field represents a mid-corner condition. The solver follows \codeword{SIM_TYPE}: a steady case uses \codeword{SRFSimpleFoam} and a transient case uses \codeword{SRFPimpleFoam}.
...
\begin{itemize}
    ...
    \item \codeword{CORNER_SOLVER} [\codeword{str}] - the OpenFOAM SRF solver used for the cornering run, the steady default is \codeword{SRFSimpleFoam}, when \codeword{SIM_TYPE} is \codeword{transient} it is auto-upgraded to \codeword{SRFPimpleFoam} (\codeword{SRFSimpleFoam}, \codeword{SRFPimpleFoam})
\end{itemize}
...
\begin{itemize}
    ...
    \item Run \codeword{caseSetup} as normal. The case is built full width, the SRF rotation is written, the wheels are set to their local corner speeds, and the run uses \codeword{CORNER_SOLVER} (\codeword{SRFSimpleFoam} for steady, \codeword{SRFPimpleFoam} for transient).
\end{itemize}
