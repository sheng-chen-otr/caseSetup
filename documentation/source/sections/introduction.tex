...
\begin{itemize}
    \item \codeword{01_incoming(YYYY-MM-DD)} - Contains all the incoming geometry from other departments or other files which are required for the simulation. Each instance of CAD delivered to the department must be in a dated folder with a short descriptive name.
    \item \codeword{02_references} - Contains all the geometry files that are used in the simulation. Sub-directories are ANSA and MSH
