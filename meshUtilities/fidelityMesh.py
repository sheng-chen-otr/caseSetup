...
def main():
    ...
    mesh_setup = createMeshSetup(fluidDomain,fullCaseSetupDict,geomDict,refTable)
    ...
    print("Creating mesh!")
    mesh_setup.start_mesh_generation(machines = hosts, num_proc = cpus)
    if fluidDomain.mesh_exists_for_domain():
        mesh_setup.print_mesh_statistics()
        print('Meshing complete! Finishing up...')
        project.save_project()
    else:
        print("Error in mesh generation, saving and exiting!")
        project.save_project()
...
def createMeshSetup(fluidDomain,fullCaseSetupDict,geomDict,refTable):
    ...
    fluidDomain = setInflationLayers(mesh_setup,geomDict,fluidDomain,fullCaseSetupDict)
    return mesh_setup
        ...
        if any(x in assy.get_name().lower() for x in nonGeomList):
            ...
            for geom in geomDict.keys():
                if assy.get_name() in geom:
                    if 'GEOMX' in assy.get_name():
                        
                        geomRef = int(geomDict[geom]['refinement'][0])
                        geomxRefLevels = float(fullCaseSetupDict[assy.get_name()]['GEOMX_MIN_MAX_LEVEL'][0])
                        geomxRefInc = int(fullCaseSetupDict[assy.get_name()]['GEOMX_FEAT_EDGE_LEVEL_INC'][0])
                        
                        minGeomxRefLevel = geomxRefLevels[0]
                        maxGeomxRefLevel = geomxRefLevels[1]
                        print('\t\t\tAttempting to set max level %s for %s' % (maxGeomxRefLevel,assy.get_name()))
                        if maxGeomxRefLevel < minRefLevel:
                            print('\t\t\t\tGeometry assigned refinement is less than global min, using global min instead!')
                            maxGeomxRefLevel = minRefLevel
                        #print('\t\t\tSetting edge proximity refinement with level %s' % (minGeomxRefLevel+geomxRefInc))
                        # meshSetup.set_auto_edge_proximity('true',assy,
                        #                                              type='Max refinement level', 
                        #                                              value=minGeomxRefLevel+geomxRefInc)
                        print('\t\t\tSetting curvature refinement max refinement level as %s' % (maxGeomxRefLevel))
                        meshSetup.set_curvature_refinement(surf, 
                                                   max_division = 4, 
                                                   min_cell_size = calculateCellSize(maxGeomxRefLevel,baseSize)
                                                   )


                    else:
                        geomRef = int(geomDict[geom]['refinement'][0])
                        geomRefInc = int(fullCaseSetupDict['GLOBAL_REFINEMENT']['FEAT_EDGE_LEVEL_INC'][0])
                        ...
                        #print('\t\t\tSetting edge proximity refinement with level %s' % (geomRef+geomRefInc))
                        #meshSetup.set_auto_edge_proximity_refinement(assy,type='Max refinement level', value=geomRef+geomRefInc)
                        


        print('\t\tSetting PID refinement levels...')
        for surf in assy.get_surfaces(): 
            for key in refTable.keys():
                ...
                if key in surf.get_name():
                    ...
                    maxCellSize = pidRefTable['maximum_surface_length']
                    minCellSize = pidRefTable['curvature_minimum_length']       
         
            try:
                print('\t\t\t%s: - Max Length: %1.4fm - Min Length %1.4fm' % (surf.get_name(),
                                                                                maxCellSize,
                                                                                minCellSize))
                meshSetup.set_uniform_refinement(surf,
                ...
                print('\t\t\tNo surface PIDs found for %s, using geometry refinement level!' % (assy.get_name()))
                print('\t\t\t%s' % (e))


    return meshSetup

def setInflationLayers(meshSetup,geomDict,fluidDomain,fullCaseSetupDict):
    meshDomain = meshSetup.get_domain('internal')
    print('\t\tSetting inflation layers...')
    for assy in fluidDomain.get_root_assemblies():
        if any(x in assy.get_name().lower() for x in nonGeomList):
            if 'domain' in assy.get_name():
                print('\t\t\tSetting inflation layers for ground...')
                print('\t\t\t\tFirst layer height: 0.001m')
                print('\t\t\t\tExpansion ratio: 1.3')
                print('\t\t\t\tN Layers: 5')
                meshDomain.set_viscous_layers_enlarge('domain|z-min', 
                                        insertion_method = 'Inflation',
                                        enable = True,
                                        first_layer_thickness=0.001, 
                                        stretching_ratio=1.3, 
                                        num_layers=5, 
                                        num_equal_first_layers=1, 
                                        )
        else:
            
            globalFirstLayerHeight = float(fullCaseSetupDict['GLOBAL_REFINEMENT']['FIRST_LAYER_HEIGHT'][0])
            globalExpansionRatio = float(fullCaseSetupDict['GLOBAL_REFINEMENT']['DEF_EX_RATIO'][0])
            print('\t\t\tGlobal first layer height: %1.5fm' % (globalFirstLayerHeight))
            print('\t\t\tGlobal expansion ratio: %1.2fm' % (globalExpansionRatio))
            for geom in geomDict.keys():
                if assy.get_name() in geom:
                    print('\t\t\t\t%s' % (assy.get_name()))
                    if 'GEOMX-' in assy.get_name():
                        
                        geomxFirstLayerHeight = float(fullCaseSetupDict[assy.get_name()]['FIRST_LAYER_HEIGHT'][0])
                        geomxLayerRatio = float(fullCaseSetupDict[assy.get_name()]['GEOMX_LAYER_RATIO'][0])
                        geomxNLayers = float(fullCaseSetupDict[assy.get_name()]['GEOMX_NLAYERS'][0])
                        print('\t\t\t\t\tFirst layer height: %1.5fm' % (geomxFirstLayerHeight))
                        print('\t\t\t\t\tExpansion ratio: %1.5fm' % (geomxLayerRatio))
                        print('\t\t\t\t\tN Layers: %s' % (geomxNLayers))
                        meshDomain.set_viscous_layers_enlarge(assy, 
                                        insertion_method = 'Inflation',
                                        enable = True,
                                        first_layer_thickness=geomxFirstLayerHeight, 
                                        stretching_ratio=geomxLayerRatio, 
                                        num_layers=geomxNLayers, 
                                        num_equal_first_layers=1, 
                                        )

                    else:
                        geomNLayers = int(geomDict[geom]['layers'][0])
                        print('\t\t\t\t\tN Layers: %s' % (geomNLayers))
                        meshDomain.set_viscous_layers_enlarge(assy, 
                                        insertion_method = 'Inflation',
                                        enable = True,
                                        first_layer_thickness=globalFirstLayerHeight, 
                                        stretching_ratio=globalExpansionRatio, 
                                        num_layers=geomNLayers, 
                                        num_equal_first_layers=1, 
                                        )
